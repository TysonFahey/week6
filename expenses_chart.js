...
function addExpenseToChart(name, amount) {
  ...
  let color = chartColors[ colorCount % chartColors.length] // % gives the remainder back 
...
// TODO add event listener to click the Add Expense button when the enter key is pressed (enter key is 13)
window.addEventListener('keyup', function() {
    if (this.event.keyCode === 13) {
        ...
        if ( inputElements.includes(document.activeElement)){
            ...
            expenseNameInput.focus()  // focus on expense type after pressing enter
        }
    }
})